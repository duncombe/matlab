...
%=========================================================================
% SW_CNDR  $Id: sw_cndr.m,v 1.6 2011-04-22 15:07:34 duncombe Exp $
...
% 2011-04-20 CMDuncombe Rae, Vectorize, removing the loops: runs ~100 times faster
...
T68 = T * 1.00024;
    ...
    while ~end_loop
       Rx_loop = Rx_loop + (S_loop - SInc)./...
           sw_salds(Rx_loop,T_loop./1.00024 - 15);
       
       SInc    = sw_sals(Rx_loop.*Rx_loop,T_loop);
       iloop   = iloop + 1;
       dels    = abs(SInc-S_loop);
       if (any(any(dels>1.0e-10)) && iloop<100)
          end_loop = 0;
       else
          end_loop = 1;
       end %if
