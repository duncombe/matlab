...
%     [format		specifies style of plot, one of 
%                 pretty, hist, density, []
%     [formats  standard matlab string for plot.m format (like '.r', also 
% 		known as pen)]
%
%     The different plotting styles take optional arguments:
%         pretty:   fourth argument toggles between showing mean direction
%         	    (true) and not showing it (false)
%         hist:     fourth argument determines number of bins/bin centers
%                   fifth argument determines whether normalized
%                   (true) or count (false) histogram is shown
...
% 	Summary:
% 
% 	circ_plot(ALPHA,'pretty',pen,meanarrow,plotargpairs)
% 
% 	circ_plot(ALPHA,'hist',pen,numbins,norm,meanarrow,plotargpairs)
% 
% 	circ_plot(ALPHA,'density',???????) appears above in the help but never
% 	in the program!
%
%   Output:
%     a         axis handle
...
% 2010/11/26 CMDR
% 	bug fixes to normalizing histogram plots
