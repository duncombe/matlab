...
DatabasesDir = '/tmp';
DatabasesDir = '/usr/local/data/topo_6.2';
DataBaseFileName='topo_6.2.img';
...
for i = 1:length(wlon);
        ...
        % Open the data file
	disp(['Reading from ' DatabasesDir '/' DataBaseFileName ]);
        fid = fopen([DatabasesDir '/' DataBaseFileName ], 'r','b');
        if (fid < 0)
                errordlg(['Could not open database: ' DatabasesDir '/' DataBaseFileName],'Error');
        end
